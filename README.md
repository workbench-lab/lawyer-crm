...
> **Демо-доступ:** `demo@lawyer.ru` / `demo1234` (подсказка есть прямо на экране входа — клик по ней заполняет форму).

## Возможности

- Экран входа с демо-авторизацией (без регистрации), сессия сохраняется.
- Таблица клиентов: аватары, имя, телефон, статус дела, дата добавления, удаление через модалку подтверждения.
- Добавление клиента с валидацией и маской телефона `+7 (900) 000-00-00`.
- Смена статуса прямо в таблице — кастомный цветной дропдаун.
- Счётчики: всего клиентов и по каждому статусу, обновляются мгновенно.
- 🔔 **Бонус:** при добавлении клиента юристу приходит красиво оформленное уведомление в Telegram со ссылкой на CRM.
- Отказоустойчивость: если Supabase недоступен, приложение автоматически переключается на localStorage и показывает предупреждение — демо работает всегда.
- Адаптив: на мобильных таблица превращается в карточки; анимации, тосты, кастомные компоненты без UI-библиотек.

## Стек и архитектура

- **Vite + React 19 + TypeScript** — быстрый старт, типобезопасность, минимум зависимостей (без роутера и стейт-менеджеров — им тут нечего делать).
- **Дизайн**: рукописный CSS с дизайн-токенами, шрифты Inter + Playfair Display (Google Fonts), иконки [lucide-react](https://lucide.dev).
